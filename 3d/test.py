...
asm_p_mod.add(clemma2.up(1.62/2+0.1).rotateZ(deg(-90)).forw(2.54*7).left(2.54*2)).set_color(0, 0.5, 0)
...
asm_p_mod.add(p_mod).set_color(0, 0, 0)
asm_p_mod.relocate(rotateX(deg(-90)) * rotateZ(deg(-90)) * down(8.4+1.7/2 + 2.5) * left(0.7) * back(-4.3))
################################
asm_stuff = zencad.assemble.unit()
asm_stuff.add(from_brep('./res/ttgo-t-energy-t18-v2.brep').rotateX(deg(-90)).moveY(-25))
#asm_stuff.relocate(forw(20.4+2.54*5) * left(9.5+2.54*3))
################################
...
p_mod = bread_board ^ box(21, 1.62, 53, True).left(2.54/2)
...
#display(p_mod.left(5).bbox().shape(), color = (0.5, 0.5, 1.0, 0.5))
show()
