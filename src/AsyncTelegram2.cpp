...
MessageType AsyncTelegram2::getNewMessage(TBMessage &message )
{
    ...
    if (getUpdates()) {
        ...
        {   // Add as scope in order to destroy updateDoc as soon as possible
            ...
            #elif defined(ESP32)
            //log_error("ESP.getMaxAllocHeap()==%u", ESP.getMaxAllocHeap());
            //DynamicJsonDocument updateDoc(ESP.getMaxAllocHeap() - BUFFER_MEDIUM);
            ...
            if (err) {
                ...
                log_error("m_rxbuffer.length()==%d\n", m_rxbuffer.length());
                //Serial.println();
                //Serial.println
                log_error("m_rxbuffer=\"%s\"\n", m_rxbuffer);
